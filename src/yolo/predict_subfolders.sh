...
INPUT_FOLDER="/workspaces/projet_bees_detection_basile/data_bees_detection/whole_dataset/inat_25_04"
...
for dir in "$INPUT_FOLDER"/*/; do
  ...
  if [ -n "$(ls -A $dir)" ]; then
    ...
    python3 /workspaces/projet_bees_detection_basile/bees_detection/src/yolo/predict.py --i "$dir"
  fi
done
...
python3 /workspaces/projet_bees_detection_basile/bees_detection/src/crop/crop_from_csv.py
