...
#from keras_yolov2.utils import print_results_p_r_f1_ap_per_classes
...
def _main_(args):
    ...
    evaluations={}  # dictionary to store the results of the evaluation for each csv file

    path += f'_{count}' # path to the directory where the pickles will be saved
    ...
    for test_csv_file in test_csv_files:
        ...
        if os.path.exists(test_csv_file):
            ...
            test_imgs, seen_test_labels = parse_annotation_csv(test_csv_file,
                                                            config['model']['labels'],
                                                            config['data']['base_path'])
            ...
            precisions,recalls,f1_scores, average_precisions=test_eval._calc_avg_precisions()

            test_name = test_csv_file.split('/')[-1].split('.')[0]
            print("For", test_name)
            print('VALIDATION LABELS: ', seen_test_labels)
            print('Final results:')
            print('\nClass metrics:')

            evaluation={}

            for i,label in enumerate(seen_test_labels):

                precision=precisions[i].mean()
                recall=recalls[i].mean()
                f1_score=f1_scores[i].mean()
                average_precision=average_precisions[i].mean()

                evaluation[label]=[precision,recall,f1_score,average_precision]
                print(f"{label}: P={precision} R={recall} F1={f1_score} AP={average_precision}")

            evaluations[f"{test_name}"]=evaluation

            # class_mean_P, class_mean_R, class_mean_F1 = print_results_metrics_per_classes(class_res, seen_test_labels)
