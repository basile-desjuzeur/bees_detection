...
from keras.layers import Activation, Conv2D, Input, MaxPooling2D, BatchNormalization, Lambda, LeakyReLU, \
    concatenate
...
MOBILENET2_BACKEND_PATH = base_path + "benchmark_weights/MobileNetV2_pretrained_iNat_42classes.h5"  # should be hosted on a server
...
class BaseFeatureExtractor(object):
    ...
    def get_output_shape(self): # en combien de cellule en largeur et en longueur yolo découpe l'image
...
class MobileNetV2Feature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, freeze, finetune, alpha=1.0, depth_multiplier=1):
        input_image = Input(shape=input_size)

        mobilenet2 = MobileNetV2(input_shape=input_size, include_top=False, alpha=alpha, weights='imagenet') #on rajoute weights='imagenet' pour utiliser les poids d'imagenet
        # if input_size[2] == 3:
        #     try:
        ...
        x = mobilenet2(input_image)#, training=not finetune)
