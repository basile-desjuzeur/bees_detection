...
def _main_(args):
    ...
    for test_csv_file in test_csv_files:
        ...
        if os.path.exists(test_csv_file):
            ...
            print(f"Proportion of true box covered by pred box on true positives: {round(intersections_global,3)}")
            ...
            pickle.dump(class_predictions, open( f"{new_path}/prediction_TP_FP_FN_{config['model']['backend']}_{test_name}.p", "wb" ) )
            pickle.dump(class_metrics, open( f"{new_path}/TP_FP_FN_{config['model']['backend']}_{test_name}.p", "wb" ) )
            pickle.dump(class_res, open( f"{new_path}/P_R_F1_{config['model']['backend']}_{test_name}.p", "wb" ) )
            pickle.dump(global_results, open( f"{new_path}/P_R_F1_global_{config['model']['backend']}_{test_name}.p", "wb" ) )  
            pickle.dump(boxes_preds, open(f"{new_path}/boxes_{config['model']['backend']}_{test_name}.p", "wb"))
